...
router.post('/save', function(req, res) {
    ...
    if (req.body.id) {
        if (linestring) {
            ...
            query = "UPDATE walks SET date = ?, start = ?, \"end\" = ?, updated_at = NOW() WHERE id = ? RETURNING *";
            values = [req.body.date, req.body.start, req.body.end, req.body.id];
        }
    }
    else {
    ...
    models.sequelize.query(query, Walk.build(), {raw : true, replacements: values}).then(function (row) {
        res.json(row);
    });
...
router.get('/export/:id', function(req, res) {
    Walk.find({id : req.params.id }).then(function (row) {
